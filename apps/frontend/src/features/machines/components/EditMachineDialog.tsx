...
import { Edit2, Server, X } from 'lucide-react';
...
export function EditMachineDialog({ machine, sections }: EditMachineDialogProps) {
  const [open, setOpen] = useState(false);
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [sectionId, setSectionId] = useState<string>(machine.sectionId.toString());
  const [name, setName] = useState(machine.name);
  const [hospCode, setHospCode] = useState(machine.hospCode || '');
  ...
  const resetFormFields = () => {
    ...
    setHospCode(machine.hospCode || '');
    setSectionId(machine.sectionId.toString());
  };
          ...
          <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-[#c41e3a]/30 dark:border-[#e84855]/40">
            <div className="flex items-center justify-between p-5 sm:p-6 border-b-2 border-[#c41e3a]/20 dark:border-[#e84855]/30 sticky top-0 bg-white dark:bg-[#1e1e1e] z-10">
              <div className="flex items-center gap-3">
                <Server size={24} className="text-[#c41e3a] dark:text-[#e84855]" fill="none" />
                <h2 className="text-gray-900 dark:text-white font-bold text-lg sm:text-xl">Edit Machine</h2>
              </div>
              <button
                type="button"
                onClick={() => {
            ...
            <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-6">
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-3 pt-6 border-t-2 border-[#c41e3a]/10 dark:border-[#e84855]/20 mt-8">
                ...
                <button
                  type="submit"
                  disabled={isPending || !name || !sectionId}
