...
export function QcTestFormDialog({ mode, initialData, machines }: QcTestFormDialogProps) {
  const [open, setOpen] = useState(false);
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  
  const [testName, setTestName] = useState(initialData?.testName || '');
  const [testType, setTestType] = useState(initialData?.testType || '');
  const [machineId, setMachineId] = useState<string>(initialData?.machineId.toString() || '');
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      let result;
      if (mode === 'create') {
        result = await createQcTest(payload);
      } else {
        result = await updateQcTest(initialData!.id, payload);
      }
  ...
  const resetFormFields = () => {
    if (mode === 'create') {
      ...
      setMachineId(initialData?.machineId.toString() || '');
    }
  };
          ...
          <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-[#c41e3a]/30 dark:border-[#e84855]/40">
            <div className="flex items-center justify-between p-5 sm:p-6 border-b-2 border-[#c41e3a]/20 dark:border-[#e84855]/30 sticky top-0 bg-white dark:bg-[#1e1e1e] z-10">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-gray-900 dark:text-white font-bold text-lg sm:text-xl">
                  {mode === 'create' ? 'Create QC Test' : 'Edit QC Test'}
                </h2>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-6">
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-3 pt-6 border-t-2 border-[#c41e3a]/10 dark:border-[#e84855]/20 mt-8">
                ...
                >
                  {isPending ? (mode === 'create' ? 'Creating...' : 'Updating...') : (mode === 'create' ? 'Create Test' : 'Update Test')}
                </button>
              </div>
            </form>
          </div>
