...
export default function UsersPage() {
  ...
  const handleSaveEdit = (updatedUser: UserType) => {
...
async function fetchUsersData(): Promise<UsersDataResponse> {
  ...
  return {
    users: mockUsers,
=======
    const success = addDoctor(formData.username, formData.password, formData.fullName, formData.email, formData.profileImage);
...
=======
  const technicians = usersList.filter((u) => u.role === 'doctor');
              ...
              <div className="flex items-start gap-4">
                {technician.profileImage ? (
                  ...
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#c41e3a] to-[#8b1e3f] dark:from-[#e84855] dark:to-[#c75b7a] flex items-center justify-center text-white flex-shrink-0 shadow-lg ring-2 ring-[#b8860b] dark:ring-[#ffd700] font-bold">
                    {technician.fullName.split(' ').map(n => n[0]).join('').toUpperCase()}
                  </div>
                )}
